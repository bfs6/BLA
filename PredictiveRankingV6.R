...
####CV Ridge Regression####
x <- cbind(Constant = 1, TeamMat, NeutralSite = ACCGamesFull$NeutralSite)
...
marginResults <- subset(marginResults, select = -c(margin_type))
...
meanResults$beta <- mean[,1]
meanResults <- filter(meanResults, margin_type == "Team")
meanResults <- subset(meanResults, select = -c(margin_type))
meanResults <- merge(meanResults, teamIDRefs, by = "team_id", all.x = T)
meanResults <- arrange(meanResults, -beta)
